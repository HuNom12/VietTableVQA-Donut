...
### 3.1. Learning Rate
Tốc độ học được thiết lập ở mức độ vừa phải ($3 \times 10^{-5}$). Với mô hình đã được Pre-train như Donut, một Learning Rate quá lớn sẽ phá vỡ các trọng số đã được học trước đó, gây ra hiện tượng Catastrophic Forgetting.
### 3.2. Epochs & Loggingnum_train_epochs=10: 
- Cho phép mô hình duyệt qua toàn bộ tập dữ liệu 15 lần. Đây là con số khởi điểm an toàn để quan sát khả năng hội tụ.
- save_strategy="epoch": Tạo một checkpoint tại ổ cứng sau mỗi vòng huấn luyện. Nếu hệ thống bị gián đoạn, quá trình train có thể được phục hồi từ Checkpoint gần nhất thay vì phải chạy lại từ đầu.
