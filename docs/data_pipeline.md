...
Hàm set_transform() đóng vai trò như một bộ lọc kích hoạt theo thời gian thực. Nó chỉ gọi hàm biến đổi transform_fn ngay tại thời điểm mô hình yêu cầu nạp một Batch dữ liệu cụ thể để tính toán.

Nhờ cơ chế này, hệ thống chỉ tiêu tốn tài nguyên cho đúng số lượng ảnh đang nằm trong Batch hiện tại. Ngay sau khi xử lý xong và hoàn tất quá trình cập nhật trọng số, vùng nhớ VRAM đó sẽ tự động được giải phóng để đón Batch tiếp theo. Đây là yếu tố then chốt giúp quá trình huấn luyện diễn ra an toàn.
...
max_length=512: Cắt cụt (Truncate) hoặc đệm (Pad) các chuỗi văn bản về cùng một độ dài cố định. Giúp các Tensor có kích thước đồng nhất để xếp thành Batch.
